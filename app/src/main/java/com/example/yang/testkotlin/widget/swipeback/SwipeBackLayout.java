package com.example.yang.testkotlin.widget.swipeback;
...
public class SwipeBackLayout extends ViewGroup {
    ...
    /**
     * 此view将Activity的view加载在自己身上，即activity的View是自己的子View，所以mask自己就行
     */
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int measuredWidth;
        int measuredHeight;
        //getMeasureWidth方法是在onMeasure后确定的，getWidth方法是在onLayout后真正显示出来的with，科普科普
        if (childCount > 0) {
    ...
    private class DragHelperCallback extends ViewDragHelper.Callback {
        ...
        public int clampViewPositionHorizontal(View child, int left, int dx) {
            ...
            if (isSwipeEnabled()) {
                if (mDirectionMode == FROM_LEFT && !Util.canViewScrollRight(innerScrollView, downX, downY, false)) {
                    //left为当前坐标点（View的左上角），移动的范围是比paddingLeft大，比width小（不比getPaddingLeft大就能向左边滑出去了...width是最右边的坐标）
        ...
        /**
         * 根据swipeBackFraction来判断是否该finished
         *
         * @param state 状态
         */
        @Override
